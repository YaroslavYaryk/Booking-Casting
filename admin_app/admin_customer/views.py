...
from .forms import (CustomerAccessForm, CustomerContactsForm, CustomerForm,
...
def add_new_customer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            customer_obj.organization_number = handle_customer.hash_info(request.POST)
            
            try:
                handle_customer.add_user_can_change(customer_obj, user_handle.get_user_by_email(request.POST["user"]))
    ...
    return render(request, 'admin/pages/customer/customer/add_update.html', {'form': form,"users": users})
...
def add_new_customer_contacts(request):
    ...
    return render(request, 'admin/pages/customer/contacts/add_update.html', {'form': form})
...
def change_details_customer_contacts(request, customer_contacts_id):
    ...
    customer_contacts = handle_customer.get_customer_contacts_by_id(customer_contacts_id)
    if request.method == 'POST':
        form = CustomerContactsForm( request.POST, request.FILES, instance=customer_contacts)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse("admin_get_all_customer_contacts"))
        else:
...
def add_new_customer_access(request):
    ...
    return render(request, 'admin/pages/customer/access/add_update.html', {'form': form})
...
def add_new_customer_request_storage(request):
    ...
    return render(request, 'admin/pages/customer/request_storage/add_update.html', {'form': form})
...
def change_details_customer_request_storage(request, request_storage_id):
    ...
    customer_request_storage = handle_customer.get_customer_request_storage_by_id(request_storage_id)
    if request.method == 'POST':
        form = CustomerRequestStorageForm( request.POST, request.FILES, instance=customer_request_storage)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse("admin_get_all_customer_requests"))
        else:
    ...
    return render(request, 'admin/pages/customer/request_storage/add_update.html', context)
