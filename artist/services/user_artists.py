...
def add_permission_to_change(artist_id, user_email, perm_type):
    artist = get_artist_by_id(artist_id)
...
def change_permission_to_change(access_id, perm_type, user):
    ...
    if perm_type_py and artist_obj.admin != perm_type_py:
        storage = ArtistRequestsStorage.objects.filter(requester=artist_obj.access, owner=user, artist=artist_obj.artist)
        if storage:
            storage_obj = storage.first()
            storage_obj.granted = True
            storage_obj.done = True
            storage_obj.save()
            ArtistRequestsStorage.objects.filter(requester=artist_obj.access, artist=artist_obj.artist).exclude(owner=user).delete()
...
def get_my_events(artist_id):
    artist = get_artist_by_id(artist_id)
    return artist.eventartists_set.all()
