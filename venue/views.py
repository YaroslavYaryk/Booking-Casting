...
from .services import (handle_request_storage, handle_venue,
                       handle_venue_contacts)
...
def get_venue_details(request, venue_id):
    ...
    if request.method == "POST":
        ...
        form = VenueContactsAddForm(request.POST, instance=venue_contacts)
        if form.is_valid():
           form.save()
        else:
            print("nvalid")
    ...
    context = {
        "venue" : venue,
        "is_allowed_to_change" : handle_venue.is_allowed_to_change(venue_id, request.user),
...
@login_required(login_url='login')
def add_user_permission_to_change_or_see_venue(request, venue_id, user_email, perm_type):
    
    try:
        handle_venue.add_permission_to_change(venue_id, user_email, perm_type)
...
def change_user_permission_to_change_or_see_venue(request, access_id, perm_type):
    venue=""
    try:
        venue = handle_venue.change_permission_to_change(access_id, perm_type, request.user)
    except Exception as er:
        print(er)
